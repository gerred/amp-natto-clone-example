...
export function useWidgetStream(streamId, options = {}) {
    const [data, setData] = React.useState([]);
    const { throttle = 0, debounce = 0, maxBuffer = 1000, compression = 1, distinctBy } = options;
    React.useEffect(() => {
        if (!streamId)
            return;
        ...
        if (compression < 1) {
            observable = observable.pipe(map(streamData => {
                if (streamData.length === 0)
                    return streamData;
                const targetSize = Math.floor(streamData.length * compression);
                if (streamData.length <= targetSize)
                    return streamData;
...
export function emitToStream(streamId, data) {
    let subject = streamRegistry.get(streamId);
    if (!subject) {
        subject = new BehaviorSubject([]);
        streamRegistry.set(streamId, subject);
...
export function emitBatchToStream(streamId, dataArray) {
    let subject = streamRegistry.get(streamId);
    if (!subject) {
        subject = new BehaviorSubject([]);
        streamRegistry.set(streamId, subject);
