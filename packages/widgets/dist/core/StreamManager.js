...
import { buffer, bufferCount, debounceTime, distinctUntilChanged, filter, map, shareReplay, switchMap, takeUntil, throttleTime, mergeMap } from 'rxjs/operators';
export class StreamManager {
    ...
    createStream(streamId, config) {
        const fullConfig = { id: streamId, ...config };
    ...
    subscribe(streamId, config = {}) {
        ...
        // Apply buffering
        if (config.buffer) {
            stream = stream.pipe(switchMap(data => new BehaviorSubject(data).pipe(bufferCount(config.buffer), map(bufferedArrays => bufferedArrays.flat()))));
        }
        return stream.pipe(distinctUntilChanged((prev, curr) => prev.length === curr.length &&
            prev[prev.length - 1]?.sequenceId === curr[curr.length - 1]?.sequenceId), shareReplay(1));
    }
    // Get windowed data (sliding window)
    getWindowedStream(streamId, windowSizeMs) {
        return this.getStream(streamId).pipe(map(data => {
            const cutoff = Date.now() - windowSizeMs;
            return data.filter(item => item.timestamp >= cutoff);
        }));
    }
    // Get aggregated data
    getAggregatedStream(streamId, aggregationFn, windowMs = 1000) {
        return this.getWindowedStream(streamId, windowMs).pipe(map(data => aggregationFn(data)), distinctUntilChanged());
    }
    ...
    getCompressedStream(streamId, compressionRatio = 0.5) {
        return this.getStream(streamId).pipe(map(data => {
            if (data.length === 0)
                return data;
    ...
    setupMetricsMonitoring(streamId) {
...
export function createRealTimeStream(streamId, source, transform) {
...
export function mergeStreams(streamIds, targetStreamId) {
    const manager = streamManager;
    manager.createStream(targetStreamId);
    const sourceStreams = streamIds.map(id => manager.getStream(id));
    merge(...sourceStreams).pipe(mergeMap(dataArray => dataArray), buffer(interval(100)), // Batch updates every 100ms
    filter(batch => batch.length > 0)).subscribe(batch => {
